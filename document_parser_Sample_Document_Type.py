...
"""
Sample Document Type Manual Document Parser Module

This module handles parsing of Sample Document Type Manuals using LlamaIndex.
"""
...
# System prompt for Sample Document Type Manual document parsing
SYSTEM_PROMPT = """
...
def submit_parsing_job_direct_api(pdf_path, api_key, vendor_api_key, vendor_model_name):
    ...
    print(f"Submitting '{file_name}' via Llama Cloud API using {vendor_model_name} for Sample Document Type Manual parsing...")
...
async def process_pdf(pdf_path, output_dir, semaphore):
    ...
    async with semaphore:
        ...
        print(f"Processing Sample Document Type Manual '{pdf_filename}'...")
            ...
            if json_result:
                try:
                    ...
                    if isinstance(json_result, dict):
                        json_result["document_type"] = "Sample Document Type Manual"
                    elif isinstance(json_result, list) and all(isinstance(item, dict) for item in json_result):
                        for item in json_result:
                            item["document_type"] = "Sample Document Type Manual"
...
def parse_document(pdf_path: str, output_dir: str = None) -> Tuple[bool, Optional[str]]:
    """
    Parse an Sample Document Type Manual document using the direct API approach for single file processing.
    ...
    print(f"Parsing Sample Document Type Manual document: {filename}")
    ...
    # Save job info
    job_info = {"filename": filename, "job_id": job_id, "document_type": "Sample Document Type Manual"}
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="Parse Sample Document Type Manual documents using direct API approach")
    ...
    print(f"Starting process for Sample Document Type Manual PDFs in: {source_directory}")
