...
"""
Office Manual Document Parser Module

This module handles parsing of Office Manuals using LlamaIndex.
"""
...
# System prompt for Office Manual document parsing
# System prompt for ISM Manual document parsing
SYSTEM_PROMPT = """
...
def submit_parsing_job_direct_api(pdf_path, api_key, vendor_api_key, vendor_model_name):
    ...
    print(f"Submitting '{file_name}' via Llama Cloud API using {vendor_model_name} for Office Manual parsing...")
...
async def process_pdf(pdf_path, output_dir, semaphore):
    ...
    async with semaphore:
        ...
        print(f"Processing Office Manual '{pdf_filename}'...")
...
def parse_document(pdf_path: str, output_dir: str = None) -> Tuple[bool, Optional[str]]:
    """
    Parse an Office Manual document using the direct API approach for single file processing.
    ...
    print(f"Parsing Office Manual document: {filename}")
    ...
    # Save job info
    job_info = {"filename": filename, "job_id": job_id, "document_type": "Office Manual"}
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="Parse Office Manual documents using direct API approach")
    ...
    print(f"Starting process for Office Manual PDFs in: {source_directory}")
