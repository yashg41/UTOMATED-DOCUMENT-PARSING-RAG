...
"""
ISM Manual Document Parser Module

This module handles parsing of ISM Manuals using LlamaIndex.
"""
...
def submit_parsing_job_direct_api(pdf_path, api_key, vendor_api_key, vendor_model_name):
    ...
    print(f"Submitting '{file_name}' via Llama Cloud API using {vendor_model_name} for ISM Manual parsing...")
...
async def process_pdf(pdf_path, output_dir, semaphore):
    ...
    async with semaphore:
        ...
        print(f"Processing ISM Manual '{pdf_filename}'...")
...
def parse_document(pdf_path: str, output_dir: str = None) -> Tuple[bool, Optional[str]]:
    """
    Parse an ISM Manual document using the direct API approach for single file processing.
    ...
    print(f"Parsing ISM Manual document: {filename}")
    ...
    # Save job info
    job_info = {"filename": filename, "job_id": job_id, "document_type": "ISM Manual"}
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="Parse ISM Manual documents using direct API approach")
    ...
    print(f"Starting process for ISM Manual PDFs in: {source_directory}")
